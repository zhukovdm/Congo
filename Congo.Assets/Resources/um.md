...
Options could have different sets of allowed values depending on the locality of the game. The order of options is not important.
...
`Server` instance should be reachable on the network prior to creating network game.
...
Currently $6$ commands are supported: `advise`, `exit`, `greet`, `help`, `move` and `show`. All of them have `man` page, which is retrieved by `help` command.
...
Current player could move pieces using `move` command. `ai` players generate moves automatically and do not use `move` command. Any transition is additionally reported. New board position is shown immediately after the transition. Wrong moves are reported.
...
Unique game identifier is reported immediately after the greeting.
