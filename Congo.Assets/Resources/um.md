...
- `--game` creates `standard` game or new game from valid `Congo FEN` or 
  connects already existing game by `id`. The unique game `id` is generated by
  the server for all newly created `standard` and `Congo FEN` games.
...
All supported commands have `man` page, which is retrieved by `help` command.
